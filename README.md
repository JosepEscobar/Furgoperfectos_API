...
1. `swift build`
1. `.build/debug/Furgoperfectos_API`
...
* `docker run -it -p 8080:8080 -v $PWD:/swift-project -w /swift-project myapp-run sh -c .build-ubuntu/release/Furgoperfectos_API`


#### Kubernetes
To deploy your application to your Kubernetes cluster, run `helm install --name myapp .` in the `/chart/FurgoperfectosAPI` directory. You need to make sure you change the `repository` variable in your `chart/FurgoperfectosAPI/values.yaml` file points to the docker image containing your runnable application.
